...
CREATE TABLE IF NOT EXISTS ClassContent (
    ClassName   VARCHAR(50) PRIMARY KEY NOT NULL,
    UserID      INTEGER,
...
# Drop the UserData table if it exists
cursor.execute("DROP TABLE IF EXISTS UserData;")

# Recreate the UserData table without the Valid column
cursor.execute("""
CREATE TABLE IF NOT EXISTS UserData (
    UserID          INTEGER PRIMARY KEY AUTOINCREMENT,
    Email           VARCHAR(320) NOT NULL,
...
connection.commit()
connection.close()
print(f"Database {db_path} has successfully been reset!")
